...
cases and deaths, wastewater concentrations, and many others – covering
different geographic regions. This can make it a challenge to find the
data stream that you are most interested in.

Data streams fall into three categories: [V5 sources](#v5-sources),
[migrating endpoints](#migrating-endpoints), and [historical
endpoints](#historical-endpoints) (which include international sources
and private endpoints requiring authentication).

## V5 sources

The V5 API is the primary interface for active epidemiological
surveillance data.

### Online documentation and EpiPortal

The online documentation lists all data sources and signals available
through the [Delphi V5
API](https://cmu-delphi.github.io/delphi-epidata/api/v5_signals.html).

For an interactive visual exploration, the [Delphi
EpiPortal](https://delphi.cmu.edu/epiportal/) lets you filter sources
and signals by disease, pathogen, geography, and date range, view live
preview charts, and copy query code.
...
is the primary metadata lookup.
...
Called with `source = ...`, it reports that source’s available signals,
supported geographic levels, reference date ranges, and version history:
...
You can also convert the metadata into a tabular summary to search
...
The V5 API uses
[`epidata_snapshot()`](https://cmu-delphi.github.io/epidatr/dev/reference/cast_api_queries.md)
to fetch data as of a given moment (latest by default) and
[`epidata_archive()`](https://cmu-delphi.github.io/epidatr/dev/reference/cast_api_queries.md)
to fetch the full revision history. See
[`vignette("epidatr")`](https://cmu-delphi.github.io/epidatr/dev/articles/epidatr.md)
...
be able to browse the list of data sources. Without tab completion, list
them directly:
...
for the argument mapping from V4 to V5.
...
original `pub_*` and `pvt_*` functions. For more information on the
datasets that are and are not moving, please visit the [Endpoints kept
for historical
reference](https://cmu-delphi.github.io/epidatr/articles/migration-guide.html#endpoints-kept-for-historical-reference)
section of the migration guide and the [Delphi V5 Sources and
Signals](https://cmu-delphi.github.io/delphi-epidata/api/v5_signals.html)
documentation.
...
function lists all endpoint functions (i.e. functions that query
particular data sources within the API) in the package and provides
brief descriptions, explicitly noting which endpoints cover non-US
locations:
...
[`cast_api_queries()`](https://cmu-delphi.github.io/epidatr/dev/reference/cast_api_queries.md)
groups
[`epidata_snapshot()`](https://cmu-delphi.github.io/epidatr/dev/reference/cast_api_queries.md),
[`epidata_archive()`](https://cmu-delphi.github.io/epidatr/dev/reference/cast_api_queries.md),
and
[`epidata()`](https://cmu-delphi.github.io/epidatr/dev/reference/cast_api_queries.md),
which share a single documentation page;
[`epidata_meta()`](https://cmu-delphi.github.io/epidatr/dev/reference/epidata_meta.md)
and
[`epidata_aux()`](https://cmu-delphi.github.io/epidatr/dev/reference/epidata_aux.md)
each have their own page, so they get their own row.

### Examples

Some endpoints contain data within the United States only, some are
international, and some are private and require additional access to
query.
...
Some private endpoints require a dedicated secret key passed via the
`auth` argument (separate from the standard Epidata API key). Store
these in your `.Renviron` file or environment variables:

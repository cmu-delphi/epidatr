...
select geographic regions. It is widely used in epidemiological
research, real-time forecasting models, and public health dashboards.
...
- [`epidata_archive()`](https://cmu-delphi.github.io/epidatr/dev/reference/cast_api_queries.md)
  fetches all versions of a dataset across time, representing the full
  revision history.
- [`epidata_aux()`](https://cmu-delphi.github.io/epidatr/dev/reference/epidata_aux.md)
  accesses source-specific auxiliary tables containing metadata,
  laboratory protocols, or additional static keys (such as NWSS
  wastewater facility descriptions).

Additionally,
[`epidata_meta()`](https://cmu-delphi.github.io/epidatr/dev/reference/epidata_meta.md)
provides access to system metadata to list available sources, signals,
geographic granularities, and date ranges.

## Basic Usage

To make a request of a particular data source at a specific point in
time, we’ll use
[`epidata_snapshot()`](https://cmu-delphi.github.io/epidatr/dev/reference/cast_api_queries.md).
This function needs the source name, signal name, and a geographic level
in order to complete a query.

Suppose we are interested in the `nssp` source, which provides access to
a [wide range
of](https://cmu-delphi.github.io/delphi-epidata/api/v5-signals/nssp.html)
...
[`epidata_snapshot()`](https://cmu-delphi.github.io/epidatr/dev/reference/cast_api_queries.md)
returns a [`tibble`](https://tibble.tidyverse.org/) (a modern
reimagining of R’s standard data frame that prints cleanly and preserves
typed columns). (Here we’re using
[`knitr::kable()`](https://rdrr.io/pkg/knitr/man/kable.html) to make it
more readable.) Each row represents one observation for the US on one
date. The location is given in the `geo_value` column, the date it
describes in the `reference_time` column, the value of the requested
signal in `value`, and the publication date in `report_time`.

The Delphi V5 API makes signals available at different geographic
levels, depending on the source. Use `epidata_meta("nssp")` for a given
source to see which geo types it supports.

To request signals for all states instead of the entire US, we use the
`geo_type` argument. This automatically returns all available data for
that geo type:
...
You can also query multiple signals in a single request by passing a
vector to `signals`:
...
The Delphi V5 API stores a historical record of all data, including
corrections and updates, which is particularly useful for accurately
backtesting forecasting models. To retrieve versioned data in
[`epidata_snapshot()`](https://cmu-delphi.github.io/epidatr/dev/reference/cast_api_queries.md),
we can use the `snapshot_date` argument, which fetches the data as it
was known on a specific date.
...
To request all versions of the data issued within a specific time range,
we use
[`epidata_archive()`](https://cmu-delphi.github.io/epidatr/dev/reference/cast_api_queries.md)
with the `report_time` argument. This parameter allows us to fetch
versions using comparison operators (e.g., `"<2025-01-15>"`) or an
...
Some sources include extra columns connected to the signal data, such as
the population served by each NWSS sewershed or site metadata.
[`epidata_aux()`](https://cmu-delphi.github.io/epidatr/dev/reference/epidata_aux.md)
retrieves this auxiliary data, either on its own or merged onto a signal
pull.

You can pull auxiliary data directly by source. To see what key columns
you can filter on for a given source, consult that source’s page in the
[V5 signals
documentation](https://cmu-delphi.github.io/delphi-epidata/api/v5_signals.html).
Named filters on these key columns can be passed through `...` to limit
the returned rows, and `columns` can be used to select specific fields:
...
You can also attach auxiliary columns directly to a signal pull by
...
If you don’t pass explicit key filters,
[`epidata_aux()`](https://cmu-delphi.github.io/epidatr/dev/reference/epidata_aux.md)
automatically infers them from the base dataset.

## Advanced queries

### Server-side key filtering

Beyond standard arguments (`source`, `signals`, `geo_type`), some
sources include extra key dimensions that categorize the data (for
example, `nwss` categorizes by `pcr_target`, and `pophive` categorizes
by `age_group`). To see what key columns are available per source,
consult that source’s page in the [V5 signals
documentation](https://cmu-delphi.github.io/delphi-epidata/api/v5_signals.html).
You can pass these extra dimensions directly as named parameters to
...
If you want to inspect the generated API request URL underlying each
function query without actually fetching data, you can pass
`dry_run = TRUE` via
...
![plot of chunk unnamed-chunk-12](img/epidatr-unnamed-chunk-12-1.png)

plot of chunk unnamed-chunk-12

`ggplot2` can also be used with `epidatr` and `maps` to [create
...
![plot of chunk unnamed-chunk-13](img/epidatr-unnamed-chunk-13-1.png)

plot of chunk unnamed-chunk-13

### Plotting revision histories

We can also visualize revision histories from
[`epidata_archive()`](https://cmu-delphi.github.io/epidatr/dev/reference/cast_api_queries.md).
Each line shows what the time series looked like as of a different
publication date:
...
![plot of chunk archive-plot](img/epidatr-archive-plot-1.png)

plot of chunk archive-plot

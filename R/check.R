...
check_epirange_like <- function(value) {
  is.character(value) || is.numeric(value) || (is.list(value) && 'from' %in% names(value) && 'to' %in% names(value))
}
...
check_epirange_param <- function(name, value, required=TRUE) {
  ...
  if (check_epirange_like(value)) {
    return()
  }
  if (!is.list || !all(sapply(sapply(values, check_epirange_like)))) {
    stop(paste0("argument ", name, " is not a epirange"))
  }
}

...
covid_hosp_facility_lookup(state = "WY", city = "New York")
```
...
```{r, eval = FALSE}
covid_hosp_state_timeseries(states = "MA", dates = "20200510")
```
...
```{r, eval = FALSE}
del <- delphi(system = "ec", epiweek = 201501)
...
```{r, eval = FALSE}
flusurv(locations = "ca", epiweeks = 202001)
```
...
```{r, eval = FALSE}
ecdc_ili(regions = "Armenia", epiweeks = 201840)
```
...
```{r, eval = FALSE}
nidss_flu(regions = "taipei", epiweeks = epirange(200901, 201301))
```
...
```{r, eval = FALSE}
nowcast(locations = "ca", epiweeks = epirange(202201, 202319))
```
...
```{r, eval = FALSE}
dengue_nowcast(locations = "pr", epiweeks = epirange(201401, 202301))
```
...
```{r, eval = FALSE}
nidss_dengue(locations = "taipei", epiweeks = epirange(200301, 201301))
```
...
```{r, eval=FALSE}
paho_dengue(regions = "ca", epiweeks = epirange(200201, 202319))
```
...
```{r, eval = FALSE}
wiki(language = "en", articles = "influenza", epiweeks = epirange(202001, 202319))
```

...
- **Historical endpoints** provide access to datasets that are static and kept for retrospective analysis:
  - **Recently migrated endpoints**: legacy endpoints that stopped receiving new data on September 30, 2026 when active surveillance transitioned to V5. This includes the V4 endpoint `pub_covidcast()` and V3 endpoints such as `pub_fluview()`, `pub_flusurv()`, and `pub_meta()`.

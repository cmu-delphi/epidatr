...
You can also use `covidcast_epidata` function to get an API object, which can help with finding sources and functions without leaving R.
Using tab completion after typing `epi$signals$` below, you can see all the available sources and signals in the API, with the format `source:signal`.
...
epidata$signals
epidata$signals$`fb-survey.fb-survey:smoothed_cli`
```
...
```{r}
epicall <- epidata$signals$`fb-survey.fb-survey:smoothed_cli`$call("state", "pa", epirange(20210405, 20210410))
epicall %>% fetch()
```

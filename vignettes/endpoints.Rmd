...
```{r, eval=FALSE}
pvt_dengue_sensors(auth = Sys.getenv("SECRET_API_AUTH_SENSORS"),
                   names = "ght",
                   locations = "ag",
...
```{r, eval=FALSE}
pvt_ght(auth = Sys.getenv("SECRET_API_AUTH_GHT"),
...
```{r, eval=FALSE}
pvt_sensors(auth = Sys.getenv("SECRET_API_AUTH_SENSORS"),
            names = "sar3",
            locations = "nat",
...
```{r, eval=FALSE}
pvt_twitter(auth = Sys.getenv("SECRET_API_AUTH_TWITTER"),
            locations = "nat",
            epiweeks = epirange(200301, 202105)
            ) %>% fetch()
```

...
You can pull auxiliary data directly by source. Two parameters can be used to control download size and speed by controlling the size of the returned data. The parameter `filtered_keys` can be used to filter the returned data to fewer rows. The parameter `columns` is used to choose which columns are returned.
...
If you don’t pass `filtered_keys`, `epidata_aux()` infers them from the base dataset. Any key column in the base dataset with a single value (like `geo_value`) is used to automatically narrow the auxiliary request, so you download only the auxiliary rows you need.
